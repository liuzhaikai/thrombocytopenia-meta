...
# @Last Modified time: 2026-01-29 15：16：00

# IL-17F rs763780  显性：T(A) 隐性：C(G)

#注： 
#1.敏感性分析首选固定效应模型
#2.甘氏图点和姓名两张图对照，标记离群点对应的姓名
#3.计算、展示P值
#4.国家亚组改为非洲/东亚/中亚的地区亚组
#5.亚组分析不展示总体合并的合并效应、异质性P值、显著性P值，展示每个亚组的合并效应、P值
...
#Rscript thrombocytopenia_meta_4_reverse.R -l mian.csv -g subgroup_children_adults.csv -s severity.csv
...
forest(meta1,lab.e="Exposure",lab.c="Control",test.overall=TRUE)#lab.e:暴露组设置自定义标签（Exposure） lab.c:为对照组设置自定义标签（Control）
...
# Dominant model subgroup_analysis_Region forest

FileName<-paste("Dominant model subgroup_analysis_Region.PDF")
pdf(file=paste(opt$outdir,"/","Dominant_Model","/",FileName,sep = ""),height=9.5,width=14)

meta1<-metabin(event.e,n.e,event.c,n.c,data=Main,
	method=opt$method,sm="OR",byvar=Main$Region,
	comb.fixed=TRUE,comb.random=TRUE,
	studlab=paste(Main$Author,Main$Year))

forest(meta1,lab.e="Exposure",lab.c="Control",test.effect.subgroup=TRUE, 
	test.overall=FALSE,overall=FALSE,overall.hetstat = FALSE,addrows.below.overall=2)

dev.off()

# Dominant model subgroup_analysis_samplesize forest

FileName<-paste("Dominant model subgroup_analysis_samplesize.PDF")
pdf(file=paste(opt$outdir,"/","Dominant_Model","/",FileName,sep = ""),height=9.5,width=14)

meta1<-metabin(event.e,n.e,event.c,n.c,data=Main,
	method=opt$method,sm="OR",byvar=Main$Sample_size_group,
	comb.fixed=TRUE,comb.random=TRUE,
	studlab=paste(Main$Author,Main$Year),method.tau = "DL")
...
FileName<-paste("Dominant model subgroup_analysis_HWE.PDF")
pdf(file=paste(opt$outdir,"/","Dominant_Model","/",FileName,sep = ""),height=9.5,width=14)

meta1<-metabin(event.e,n.e,event.c,n.c,data=Main,
	method=opt$method,sm="OR",byvar=Main$HWE,
	comb.fixed=TRUE,comb.random=TRUE,
	studlab=paste(Main$Author,Main$Year))
...
n.e<-Sub$AA_AG_case+Sub$GG_case
event.c<-Sub$AG_GG_control
n.c<-Sub$AA_AG_control+Sub$GG_control
...
event.e<-Severity$AG_GG_severe
n.e<-Severity$AA_AG_severe+Severity$GG_severe
event.c<-Severity$AG_GG_not_severe
...
forest(meta1,lab.e="Severe",lab.c="Not_Severe",test.overall=TRUE)
...
# Heterozygote model subgroup_analysis_Region forest

FileName<-paste("Heterozygote model subgroup_analysis_Region.PDF")
...
FileName<-paste("Heterozygote model subgroup_analysis_samplesize.PDF")
pdf(file=paste(opt$outdir,"/","Heterozygote_Model","/",FileName,sep = ""),height=9.5,width=14)
...
FileName<-paste("Heterozygote model subgroup_analysis_HWE.PDF")
pdf(file=paste(opt$outdir,"/","Heterozygote_Model","/",FileName,sep = ""),height=9.5,width=14)
...
event.e<-Sub$AG_case
n.e<-Sub$AG_case+Sub$AA_case
event.c<-Sub$AG_control
n.c<-Sub$AG_control+Sub$AA_control
...
n.e<-Severity$AG_severe+Severity$AA_severe
event.c<-Severity$AG_not_severe
n.c<-Severity$AG_not_severe+Severity$AA_not_severe
...
FileName<-paste("Heterozygote model sensetivity_analysis_common_severity.PDF")
pdf(file=paste(opt$outdir,"/","Heterozygote_Model","/",FileName,sep = ""),height=3.5,width=10)

forest(metainf(meta1,pooled="common"),lab.e="Not_Severe",lab.c="Severe")
...
#敏感性分析，注意是用固定效应模型还是随机效应模型
FileName<-paste("Homozygote model sensetivity_analysis_common.PDF")
pdf(file=paste(opt$outdir,"/","Homozygote_Model","/",FileName,sep = ""),height=3.5,width=10)

forest(metainf(meta1,pooled="common"),lab.e="Exposure",lab.c="Control")
...
# Homozygote model subgroup_analysis_Region forest

FileName<-paste("Homozygote model subgroup_analysis_Region.PDF")
...
FileName<-paste("Homozygote model subgroup_analysis_samplesize.PDF")
pdf(file=paste(opt$outdir,"/","Homozygote_Model","/",FileName,sep = ""),height=9.5,width=14)
...
FileName<-paste("Homozygote model subgroup_analysis_HWE.PDF")
pdf(file=paste(opt$outdir,"/","Homozygote_Model","/",FileName,sep = ""),height=9.5,width=14)
...
event.e<-Sub$GG_case
n.e<-Sub$AA_case+Sub$GG_case
event.c<-Sub$GG_control
n.c<-Sub$AA_control+Sub$GG_control
...
event.e<-Severity$GG_severe
n.e<-Severity$AA_severe+Severity$GG_severe
event.c<-Severity$GG_not_severe
...
FileName<-paste("Homozygote model sensetivity_analysis_common_severity.PDF")
pdf(file=paste(opt$outdir,"/","Homozygote_Model","/",FileName,sep = ""),height=3.5,width=10)
...
# Recessive model subgroup_analysis_Region forest

FileName<-paste("Recessive model subgroup_analysis_Region.PDF")
...
FileName<-paste("Recessive model subgroup_analysis_samplesize.PDF")
pdf(file=paste(opt$outdir,"/","Recessive_Model","/",FileName,sep = ""),height=9.5,width=14)
...
FileName<-paste("Recessive model subgroup_analysis_HWE.PDF")
pdf(file=paste(opt$outdir,"/","Recessive_Model","/",FileName,sep = ""),height=9.5,width=14)
...
event.e<-Sub$GG_case
n.e<-Sub$AA_case+Sub$AG_GG_case
event.c<-Sub$GG_control
n.c<-Sub$AA_control+Sub$AG_GG_control
...
FileName<-paste("Recessive model sensetivity_analysis_common_severity.PDF")
pdf(file=paste(opt$outdir,"/","Recessive_Model","/",FileName,sep = ""),height=3.5,width=10)
...
# Allele model subgroup_analysis_Region forest

FileName<-paste("Allele model subgroup_analysis_Region.PDF")
...
# Allele model subgroup_analysis_samplesize forest

FileName<-paste("Allele model subgroup_analysis_samplesize.PDF")
pdf(file=paste(opt$outdir,"/","Allele_Model","/",FileName,sep = ""),height=9.5,width=14)
...
# Allele model subgroup_analysis_HWE forest

FileName<-paste("Allele model subgroup_analysis_HWE.PDF")
pdf(file=paste(opt$outdir,"/","Allele_Model","/",FileName,sep = ""),height=9.5,width=14)
...
event.e<-Sub$G_case
n.e<-Sub$A_case+Sub$G_case
event.c<-Sub$G_control
n.c<-Sub$A_control+Sub$G_control
...
event.e<-Severity$G_severe
n.e<-Severity$A_severe+Severity$G_severe
event.c<-Severity$G_not_severe
...
FileName<-paste("Allele model sensetivity_analysis_common_severity.PDF")
pdf(file=paste(opt$outdir,"/","Allele_Model","/",FileName,sep = ""),height=3.5,width=10)
