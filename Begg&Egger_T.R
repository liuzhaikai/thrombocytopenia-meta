...
#计算Begg检验和Egger检验
begg_test <- metabias(meta1, method.bias = "rank",k.min=9)
egger_test <- metabias(meta1, method.bias = "linreg",k.min=9)

#直接输出简洁的P值结果
cat("--- 显性模型Begg秩相关检验 ---\n")
cat("P值 =", begg_test$p.value, "\n\n")
cat("--- 显性模型Egger线性回归检验 ---\n")
cat("P值 =", egger_test$p.value, "\n")
